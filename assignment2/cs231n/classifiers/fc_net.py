...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ############################################################################
        # added to each row of activation in LY1
        self.params["b1"] = np.zeros(hidden_dim)
        # added to each row of activations in LY2
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        Z1, cache_Z1 = affine_forward(
            x=X, w=self.params["W1"], b=self.params["b1"])
        A1, cache_A1 = relu_forward(x=Z1)
        scores, cache_scores = affine_forward(
            x=A1, w=self.params["W2"], b=self.params["b2"])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        loss, dscores = softmax_loss(x=scores, y=y)
        # bias is not regularized, as they are similar to fixed intercepts
        loss += self.reg/2*(np.sum(self.params["W2"]**2))
        loss += self.reg/2*(np.sum(self.params["W1"]**2))
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        # copy neccessary or changes to all_dims apply to hidden_dims as well
        all_dims = hidden_dims.copy()
        all_dims.insert(0, input_dim)
        all_dims.append(num_classes)
        for i in range(self.num_layers):
            weight_matrix = "W{}".format(i+1)
            bias = "b{}".format(i+1)
            gamma = "gamma{}".format(i+1)
            beta = "beta{}".format(i+1)
            dim_in = all_dims[i]
            dim_out = all_dims[i+1]
            self.params[weight_matrix] = weight_scale * np.random.randn(dim_in, dim_out)
            self.params[bias] = np.zeros(dim_out)
            if self.use_batchnorm and i != self.num_layers-1: #no bn in last layer
                self.params[gamma] = np.ones(dim_out)
                self.params[beta] = np.zeros(dim_out)
            #print("Weight dimensions:",self.params[weight_matrix].shape)
        #print("Layer weights",self.params.keys())
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            layer_input = out
        scores, cache["aff{}".format(self.num_layers)] = affine_forward(
            x=layer_input, w=self.params["W{}".format(self.num_layers)],
            b=self.params["b{}".format(self.num_layers)])
        #print("Layer caches",cache.keys())
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        penalty = 0
        for i in range(self.num_layers):
            penalty += 0.5*self.reg*np.sum(self.params["W{}".format(i+1)]**2)
        loss, dscores = softmax_loss(x=scores, y=y)
        loss += penalty
        dx, grads["W{}".format(self.num_layers)], grads["b{}".format(self.num_layers)] = affine_backward(dout=dscores, cache=cache["aff{}".format(self.num_layers)])
